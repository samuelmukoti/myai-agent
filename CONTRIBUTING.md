# Contributing to MyAIOne Agent

Thank you for contributing to MyAIOne Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want MyAIOne to work everywhere.
...
Bundled skills (in `skills/`) ship with every MyAIOne install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `myai skills browse` (labeled "official") and install it with `myai skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `myai skills install`.
...
```bash
git clone --recurse-submodules https://github.com/samuelmukoti/myai-agent.git
cd hermes-agent
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/myai" ~/.local/bin/myai
...
The user may skip setup and keep loading the skill. MyAIOne only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `myai setup` or update `~/.hermes/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing MyAIOne tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `myai --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

MyAIOne uses a data-driven skin system — no code changes needed to add a new skin.
...
MyAIOne runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
MyAIOne has terminal access. Security matters.
...
2. **Test manually**: Run `myai` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/samuelmukoti/myai-agent/issues)
- Include: OS, Python version, MyAIOne version (`myai version`), full error traceback
