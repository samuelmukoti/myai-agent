...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    myai_home = get_myai_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(myai_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(myai_home / rel for rel in _SENSITIVE_HERMES_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal MyAIOne path and cannot be attached")
