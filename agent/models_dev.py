...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.myai/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: MyAIOne ↔ models.dev
# ---------------------------------------------------------------------------

# MyAIOne provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → MyAIOne (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_myai_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a MyAIOne provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal MyAIOne agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a MyAIOne provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve MyAIOne ID → models.dev ID
...
def get_model_info(
    ...
    Accepts MyAIOne or models.dev provider ID.  Tries exact match then
