...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``MYAI_PLATFORM`` or
            ``MYAI_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("MYAI_PLATFORM")
        or get_session_env("MYAI_SESSION_PLATFORM")
    )
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.myai/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.myai/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    myai = metadata.get("myai") or metadata.get("hermes") or {}
    if not isinstance(myai, dict):
        myai = {}
    return {
        "fallback_for_toolsets": myai.get("fallback_for_toolsets", []),
        "requires_toolsets": myai.get("requires_toolsets", []),
        "fallback_for_tools": myai.get("fallback_for_tools", []),
        "requires_tools": myai.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    myai = metadata.get("myai") or metadata.get("hermes")
    if not isinstance(myai, dict):
        return []
    raw = myai.get("config")
    if not raw:
