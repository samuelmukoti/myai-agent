...
# Uses get_myai_home() so each profile gets its own Matrix store.
from myai_constants import get_myai_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("MYAI_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("MYAI_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
