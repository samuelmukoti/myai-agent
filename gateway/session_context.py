"""
Session-scoped context variables for the MyAIOne gateway.

Replaces the previous ``os.environ``-based session state
(``MYAI_SESSION_PLATFORM``, ``MYAI_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["MYAI_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("MYAI_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("MYAI_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("MYAI_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("MYAI_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("MYAI_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("MYAI_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("MYAI_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("MYAI_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("MYAI_SESSION_KEY", default=_UNSET)

_VAR_MAP = {
    "MYAI_SESSION_PLATFORM": _SESSION_PLATFORM,
    "MYAI_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "MYAI_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "MYAI_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "MYAI_SESSION_USER_ID": _SESSION_USER_ID,
    "MYAI_SESSION_USER_NAME": _SESSION_USER_NAME,
    "MYAI_SESSION_KEY": _SESSION_KEY,
}
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``MYAI_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("MYAI_SESSION_*", default)``.
