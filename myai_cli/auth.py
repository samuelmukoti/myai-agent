...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `myai model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("MYAI_OAUTH_TRACE", "").strip().lower()
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `myai logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'myai doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'myai model' for available providers, or run 'myai doctor' to diagnose config issues."
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("MYAI_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `myai auth list` / `myai status`."""
    try:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `myai auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `myai auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `myai auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `myai auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `myai auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("MYAI_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = (
        os.getenv("MYAI_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("MYAI_PORTAL_BASE_URL")
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``myai auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``myai auth add nous --label <name>``).  It gets embedded in the
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `myai status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``myai auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `myai auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `myai auth` and
    # `myai model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("MYAI_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("MYAI_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set MYAI_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _prompt_model_selection(
    ...
    """
    from myai_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from myai_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'myai model' or 'myai setup' instead."""
    print("The 'myai login' command has been removed.")
    print("Use 'myai auth' to manage credentials,")
    print("'myai model' to select a provider, or 'myai setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    ...
    if cli_tokens:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("MYAI_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `myai model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `myai model` again to switch to Nous Portal.")
            return
