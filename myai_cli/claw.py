"""myai claw — OpenClaw migration commands.
...
Usage:
    ...
    myai claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    myai claw cleanup              # Archive leftover OpenClaw directories
    myai claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'myai stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_myaione.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_myaione", script_path)
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: myai claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: myai claw migrate --yes")
            return
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: myai claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: myai claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  myai claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  myai claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  myai config set OPENROUTER_API_KEY sk-or-v1-...")
