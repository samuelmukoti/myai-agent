...
- myai config edit     - Open config in editor
- myai config set      - Set a specific value
- myai config wizard   - Re-run setup wizard
"""
...
from myai_cli.default_soul import DEFAULT_SOUL_MD
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "myai update"
...
def format_managed_message(action: str = "modify this MyAIOne installation") -> str:
    ...
    return (
        f"Cannot {action}: this MyAIOne installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall MyAIOne."
    )
...
def _secure_dir(path):
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("MYAI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When MyAIOne runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("MYAI_CONTAINER") or os.environ.get("MYAI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set MYAI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when MyAIOne
    ...
    "browser": {
        ...
        "camofox": {
            # When true, MyAIOne sends a stable profile-scoped userId to Camofox
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the MyAIOne auth store)",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote MyAIOne API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote MyAIOne API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote MyAIOne API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # MYAI_TOOL_PROGRESS and MYAI_TOOL_PROGRESS_MODE are deprecated —
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'myai setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — MyAIOne won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'myai doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("MYAI_TOOL_PROGRESS")
            old_mode = get_env_value("MYAI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from MYAI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from MYAI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("MYAI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from MYAI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: myai config set <key> <value>")
...
#   openai-codex (OAuth — myai auth) — OpenAI Codex
#   nous         (OAuth — myai auth) — Nous Portal
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real MyAIOne env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known MyAIOne vars that are no longer in .env
...
def show_config():
    ...
    print(color("  myai config edit     # Edit config file", Colors.DIM))
    print(color("  myai config set <key> <value>", Colors.DIM))
    print(color("  myai setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: myai config set <key> <value>")
            print()
            print("Examples:")
            print("  myai config set model anthropic/claude-sonnet-4")
            print("  myai config set terminal.backend docker")
            print("  myai config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'myai config migrate' to add them")
        ...
        print("  myai config           Show current configuration")
        print("  myai config edit      Open config in editor")
        print("  myai config set <key> <value>   Set a config value")
        print("  myai config check     Check for missing/outdated config")
        print("  myai config migrate   Update config with new options")
        print("  myai config path      Show config file path")
        print("  myai config env-path  Show .env file path")
        sys.exit(1)
