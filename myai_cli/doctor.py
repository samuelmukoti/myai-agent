...
Diagnoses issues with MyAIOne Agent setup.
"""
...
from myai_cli.config import get_project_root, get_myai_home, get_env_path
from myai_constants import display_myai_home

PROJECT_ROOT = get_project_root()
MYAI_HOME = get_myai_home()
_DHH = display_myai_home()  # user-facing display path (e.g. ~/.myai or ~/.myai/profiles/coder)

# Load environment variables from ~/.myai/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.myai/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    os.environ.setdefault("MYAI_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 MyAIOne Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.myai/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'myai setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'myai setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'myai setup' to create one")
                issues.append("Run 'myai setup' to create .env")
    
    # Check ~/.myai/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from myai_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'myai setup' to migrate config")
                else:
                    issues.append("Run 'myai doctor --fix' or 'myai setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'myai doctor --fix'")
        except Exception:
            pass

        # Validate config structure (catches malformed custom_providers, etc.)
        try:
            from myai_cli.config import validate_config_structure
    ...
    for subdir_name in expected_subdirs:
        subdir_path = myai_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give MyAIOne a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# MyAIOne Agent Persona\n\n"
                "<!-- Edit this file to customize how MyAIOne communicates. -->\n\n"
                "You are MyAIOne, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = myai_home / "memories"
    ...
    state_db_path = myai_home / "state.db"
    ...
    wal_path = myai_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'myai doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'myai setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: myai skills list)")

    from myai_cli.config import get_env_value
    ...
    try:
        ...
        _mem_cfg_path = MYAI_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: myai memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: myai memory setup")
                issues.append("No Honcho API key — run 'myai memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run myai memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: myai memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: myai memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from myai_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "myai -p" in content:
                            _m = _re.search(r"myai -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'myai doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
