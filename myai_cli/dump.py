...
Outputs a compact, plain-text summary of the user's MyAIOne setup
...
from myai_cli.config import get_myai_home, get_env_path, get_project_root, load_config
...
def _gateway_status() -> str:
    ...
    try:
        from myai_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(myai_home: Path) -> int:
...
def _cron_summary(myai_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = myai_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"myai_home:      {display_myai_home()}")
    ...
    toolsets = config.get("toolsets", ["myai-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(myai_home)}")
    lines.append(f"  skills:             {_count_skills(myai_home)}")
