...
Handles: myai gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "myai_cli.main --profile",
        "myai_cli.main -p",
        "myai_cli/main.py gateway",
        "myai_cli/main.py --profile",
        "myai_cli/main.py -p",
        "myai gateway",
        "gateway/run.py",
    ]
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"MYAI_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "MYAI_HOME=" in command and f"MYAI_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``myai update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            MyAIOne profile are returned.
    """
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `myai gateway`")
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy MyAIOne gateway units.
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy MyAIOne gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy MyAIOne gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy MyAIOne gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy MyAIOne gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy MyAIOne gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: myai gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo myai gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    myai gateway uninstall")
    print_info("    sudo myai gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so MyAIOne can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo myai gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo myai gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo myai gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode MyAIOne often sets ``HOME`` to a profile-scoped directory, but
...
User={username}
Group={group_name}
ExecStart={python_path} -m myai_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current MyAIOne install")
    return True
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("MYAI_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}myai gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}myai gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}myai gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}myai gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}myai gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}myai gateway start{scope_flag}")
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current MyAIOne install")
    return True
...
def launchd_stop():
    ...
    # `myai gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current MyAIOne install")
    else:
        print("⚠ Service definition is stale relative to the current MyAIOne install")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and MyAIOne will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where MyAIOne delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your MyAIOne agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address MyAIOne will use (e.g., hermes@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. MyAIOne connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: myai pairing generate bluebubbles",
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'myai pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: myai pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'myai gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from myai_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from myai_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `myai gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `myai pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `myai pairing approve`.")
    elif access_idx == 1:
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: myai gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  myai gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: myai gateway run")
                print_info("  For persistence:   tmux new -s hermes 'myai gateway run'")
        ...
        print()
        print_info("No platforms configured. Run 'myai gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: myai gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: myai gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'myai gateway run'")
                print()
            ...
            print()
            print("To run the gateway: myai gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: myai gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: myai gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: myai gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: myai gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  myai gateway install")
                    print("  sudo myai gateway install --system")
            else:
        ...
        # Stop, disable, and remove legacy MyAIOne gateway unit files from
