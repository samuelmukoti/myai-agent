#!/usr/bin/env python3
"""
MyAIOne CLI - Main entry point.

Usage:
    myai                     # Interactive chat (default)
    myai chat                # Interactive chat
    myai gateway             # Run gateway in foreground
    myai gateway start       # Start gateway as service
    myai gateway stop        # Stop gateway service
    myai gateway status      # Show gateway status
    myai gateway install     # Install gateway service
    myai gateway uninstall   # Uninstall gateway service
    myai setup               # Interactive setup wizard
    myai logout              # Clear stored authentication
    myai status              # Show status of all components
    myai cron                # Manage cron jobs
    myai cron list           # List cron jobs
    myai cron status         # Check if cron scheduler is running
    myai doctor              # Check configuration and dependencies
    myai honcho setup                    # Configure Honcho AI memory integration
    myai honcho status                   # Show Honcho config and connection status
    myai honcho sessions                 # List directory → session name mappings
    myai honcho map <name>               # Map current directory to a session name
    myai honcho peer                     # Show peer names and dialectic settings
    myai honcho peer --user NAME         # Set user peer name
    myai honcho peer --ai NAME           # Set AI peer name
    myai honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    myai honcho mode                     # Show current memory mode
    myai honcho mode [hybrid|honcho|local]  # Set memory mode
    myai honcho tokens                   # Show token budget settings
    myai honcho tokens --context N       # Set session.context() token cap
    myai honcho tokens --dialectic N     # Set dialectic result char cap
    myai honcho identity                 # Show AI peer identity representation
    myai honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    myai honcho migrate                  # Step-by-step migration guide: OpenClaw native → MyAIOne + Honcho
    myai version             Show version
    myai update              Update to latest version
    myai uninstall           Uninstall MyAIOne Agent
    myai acp                 Run as an ACP server for editor integration
    myai sessions browse     Interactive session picker with search

    myai claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (myai tools, myai setup, myai model) use
...
def _has_any_provider_configured() -> bool:
    ...
    # Only count these if MyAIOne has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants MyAIOne to use their tokens.
...
def _print_tui_exit_summary(session_id: Optional[str]) -> None:
    ...
    print(f"  myai --tui --resume {target}")
    if title:
        print(f'  myai --tui -c "{title}"')
    print()
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'myai sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like MyAIOne isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  myai setup")
        print()
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'myai setup' at any time to configure.")
        sys.exit(1)
...
def cmd_gateway(args):
    ...
    from myai_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with MyAIOne?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: myai gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        print("⚠ Pairing may not have completed. Run 'myai whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``myai model``) and the setup wizard
    ...
    from myai_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _clear_stale_openai_base_url():
    ...
    """
    from myai_cli.config import get_env_value, save_env_value, load_config
...
#
# MyAIOne uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `myai model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `myai model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  MyAIOne only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``myai model`` flow, then route aux tasks to them here.
    """
    from myai_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from myai_cli.auth import _prompt_model_selection
    from myai_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from myai_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from myai_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from myai_cli.nous_subscription import prompt_enable_tool_gateway
    import argparse
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from myai_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from myai_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `myai auth` stores device_code tokens),
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from myai_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from myai_cli.auth import _save_model_choice, deactivate_provider
    from myai_cli.config import get_env_value, load_config, save_config
    ...
    from myai_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `myai model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from myai_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from myai_cli.config import save_env_value, load_config, save_config
    from myai_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from myai_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from myai_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates MyAIOne turns to `copilot --acp`.")
    print("  MyAIOne currently starts its own ACP subprocess for each request.")
    print("  MyAIOne uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "ollama-cloud":
        from myai_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from myai_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'myai model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate MyAIOne CLI with a provider."""
    from myai_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from myai_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from myai_cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from myai_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from myai_cli.webhook import webhook_command
...
def cmd_doctor(args):
    ...
    from myai_cli.doctor import run_doctor
...
def cmd_config(args):
    ...
    from myai_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up MyAIOne home directory to a zip file."""
    if getattr(args, "quick", False):
        from myai_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a MyAIOne backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"MyAIOne Agent v{__version__} ({__release_date__})")
    ...
    try:
        from myai_cli.banner import check_for_updates
        from myai_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall MyAIOne Agent."""
    _require_tty("uninstall")
    from myai_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``myai update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update MyAIOne Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/samuelmukoti/myai-agent/archive/refs/heads/{branch}.zip"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if MyAIOne behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but MyAIOne couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but MyAIOne couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if MyAIOne behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `myai update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/samuelmukoti/myai-agent.git",
    "git@github.com:samuelmukoti/myai-agent.git",
    "https://github.com/samuelmukoti/myai-agent",
    "git@github.com:samuelmukoti/myai-agent",
}
OFFICIAL_REPO_URL = "https://github.com/samuelmukoti/myai-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official MyAIOne repository.")
        print("  This means you may miss updates from samuelmukoti/myai-agent.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/samuelmukoti/myai-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/samuelmukoti/myai-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``myai update``, every profile is now current.
    """
    homes = []
...
class _UpdateOutputStream:
    """Stream wrapper used during ``myai update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``myai update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``myai update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``myai update --gateway``) the update is already
...
def cmd_update(args):
    """Update MyAIOne Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from myai_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update MyAIOne Agent")
        return
    ...
    try:
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'myai config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: myai config migrate")
            else:
                print()
                print("Skipped. Run 'myai config migrate' later to configure.")
        else:
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: myai gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: myai -p <profile> gateway run  for each profile)"
                        )
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy MyAIOne gateway unit(s) detected:")
        ...
        print("  myai model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``myai -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → myai -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  myai profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     myai -p {name} chat")
                else:
        ...
        from myai_cli.profiles import profile_exists
        ...
        from myai_cli.profiles import rename_profile
        ...
        from myai_cli.profiles import export_profile
        ...
        from myai_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    from myai_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from myai_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter MyAIOne log files."""
    from myai_cli.logs import tail_log, list_logs
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    myai                        Start interactive chat
    myai chat -q "Hello"        Single query mode
    myai -c                     Resume the most recent session
    myai -c "my project"        Resume a session by name (latest in lineage)
    myai --resume <session_id>  Resume a specific session by ID
    myai setup                  Run setup wizard
    myai logout                 Clear stored authentication
    myai auth add <provider>    Add a pooled credential
    myai auth list              List pooled credentials
    myai auth remove <p> <t>    Remove pooled credential by index, id, or label
    myai auth reset <provider>  Clear exhaustion status for a provider
    myai model                  Select default model
    myai config                 View configuration
    ...
    myai sessions browse        Interactive session picker
    myai sessions rename ID T   Rename/title a session
    myai logs                   View agent.log (last 50 lines)
    myai logs -f                Follow agent.log in real time
    myai logs errors            View errors.log
    myai logs --since 1h        Lines from the last hour
    myai debug share             Upload debug report for support
    myai update                 Update to latest version
...
""",
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Force the first-run onboarding flow: launches the guided `myai setup` "
            "wizard and then drops into the TUI when setup completes. Designed for "
            "DevStation / MyAIOne Rig containers so a fresh shell can hand off cleanly."
        ),
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with MyAIOne Agent",
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure MyAIOne Agent with an interactive wizard. "
        "Run a specific section: myai setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for MyAIOne CLI",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of MyAIOne Agent components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with MyAIOne Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your MyAIOne setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for MyAIOne Agent. Use 'myai debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    myai debug share              Upload debug report and print URL
    myai debug share --lines 500  Include more log lines
    myai debug share --expire 30  Keep paste for 30 days
    myai debug share --local      Print report locally (no upload)
    myai debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'myai debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up MyAIOne home directory to a zip file",
        description="Create a zip archive of your entire MyAIOne configuration, "
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a MyAIOne backup from a zip file",
        description="Extract a previously created MyAIOne backup into your "
        "MyAIOne home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage MyAIOne Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from myai_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from myai_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from myai_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from myai_cli.plugins_cmd import plugins_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'myai tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # myai tools disable <name...> [--platform cli]
    ...
    # myai tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from myai_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from myai_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run MyAIOne as an MCP server",
        description=(
            "Manage MCP server connections and run MyAIOne as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'myai mcp add' to connect to a new server, or\n"
            "'myai mcp serve' to expose MyAIOne conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run MyAIOne as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch myai --resume <id> by replacing the current process
            ...
            if myai_bin:
                ...
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "myai_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to MyAIOne",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to MyAIOne",
    ...
    def cmd_claw(args):
        from myai_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update MyAIOne Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall MyAIOne Agent",
        description="Remove MyAIOne Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run MyAIOne Agent as an ACP (Agent Client Protocol) server",
        description="Start MyAIOne Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch MyAIOne Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated MyAIOne instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the MyAIOne Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter MyAIOne log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    myai logs                    Show last 50 lines of agent.log
    myai logs -f                 Follow agent.log in real time
    myai logs errors             Show last 50 lines of errors.log
    myai logs gateway -n 100     Show last 100 lines of gateway.log
    myai logs --level WARNING    Only show WARNING and above
    myai logs --session abc123   Filter by session ID
    myai logs --component tools  Only show tool-related lines
    myai logs --since 1h         Lines from the last hour
    myai logs --since 30m -f     Follow, starting from 30 min ago
    myai logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``myai -c Pokemon Agent Dev`` → ``myai -c 'Pokemon Agent Dev'``
    ...
    from myai_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'myai -c model' where 'model' is consumed as the
