"""
MCP Server Management CLI — ``myai mcp`` subcommand.

Implements ``myai mcp add/remove/list/test/configure`` for interactive
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from myai_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  myai mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  myai mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  myai mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: myai mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `myai mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  myai mcp add <name> --url <endpoint>')
        _info('  myai mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `myai mcp remove` + `myai mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'myai mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``myai mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("myai mcp serve                              Run as MCP server")
        _info("myai mcp add <name> --url <endpoint>        Add an MCP server")
        _info("myai mcp add <name> --command <cmd>         Add a stdio server")
        _info("myai mcp add <name> --preset <preset>       Add from a known preset")
        _info("myai mcp remove <name>                      Remove a server")
        _info("myai mcp list                               List servers")
        _info("myai mcp test <name>                        Test connection")
        _info("myai mcp configure <name>                   Toggle tools")
        _info("myai mcp login <name>                       Re-authenticate OAuth")
        print()
