...
- ``myai_cli.providers``        -- canonical provider identity + overlays
- ``myai_cli.model_normalize``  -- per-provider name formatting
...
from myai_cli.providers import (
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with MyAIOne Agent. They lack the tool-calling capabilities "
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'myai model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'myai doctor' — config issues detected:"
    ...
    # Mirrors the same logic in myai_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from myai_cli/models.py (OPENROUTER_MODELS,
    ...
    from myai_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # --- 1. Check MyAIOne-mapped providers ---
    ...
    # --- 2. Check MyAIOne-only providers (nous, openai-codex, copilot, opencode-go) ---
    from myai_cli.providers import HERMES_OVERLAYS
    from myai_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → MyAIOne provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use MyAIOne IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve MyAIOne slug — e.g. "github-copilot" → "copilot"
        ...
        # Use curated list — look up by MyAIOne slug, fall back to overlay key
    ...
    # with `myai model`).
    try:
        from myai_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
