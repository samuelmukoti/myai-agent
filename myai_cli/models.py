...
Add, remove, or reorder entries here — both `myai setup` and
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `myai model`
    flow without maintaining a separate static list.
    """
    from myai_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from myai_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  myai model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `myai model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `myai model` TUI
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Nous Research subscription — upstream MyAIOne users)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``myai model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``myai auth add openai-codex`` without
    ``myai model``).
    """
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``myai model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from myai_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def detect_provider_for_model(
    ...
    if direct_match:
        ...
        if not has_creds:
            try:
                from myai_cli.auth import _load_auth_store
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to MyAIOne's canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``myai_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether MyAIOne should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from myai_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from myai_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from myai_cli.copilot_auth import copilot_request_headers
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: MyAIOne's default Claude IDs elsewhere use
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from myai_constants import AI_GATEWAY_BASE_URL
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_myai_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"MyAIOne will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
