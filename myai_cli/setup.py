"""
Interactive setup wizard for MyAIOne Agent.
...
_DOCS_BASE = "https://docs.myai1.ai/docs"
...
from myai_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure MyAIOne using environment variables or config commands:")
    print_info("  myai config set model.provider custom")
    print_info("  myai config set model.base_url http://localhost:8080/v1")
    print_info("  myai config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'myai setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from myai_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from myai_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'myai setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'myai setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'myai setup terminal'"))
    ...
    print()
    print(f"   {color('myai setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('myai setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('myai setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('myai setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('myai setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('myai config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('myai config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('myai config set <key> <value>', Colors.GREEN)}")
...
# between `myai tools` and `myai setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``myai model``)
    ...
    provider added to ``myai model`` is automatically available here.
    ...
    # Delegate to the shared myai model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: myai model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from myai_cli.auth_commands import auth_add_command
            ...
            print_info(
                "MyAIOne can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one MyAIOne can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'myai setup' or configure AUXILIARY_VISION_* settings")
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'myai setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where MyAIOne runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using MyAIOne via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("MYAI_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `myai setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("MYAI_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("MYAI_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where MyAIOne delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://docs.myai1.ai/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where MyAIOne delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where MyAIOne delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'myai whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'myai whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from myai_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from myai_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from myai_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from myai_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from myai_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from myai_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from myai_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from myai_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects MyAIOne to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from myai_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://docs.myai1.ai/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://docs.myai1.ai/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  myai config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with MyAIOne from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'myai setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     myai config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: myai gateway install")
            else:
                print_info("  You can install later: myai gateway install")
                if supports_systemd:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   myai gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `myai setup tools` and `myai tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from myai_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from myai_cli.auth import get_active_provider
...
# Config values may have different semantics between OpenClaw and MyAIOne.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure MyAIOne to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point MyAIOne at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point MyAIOne at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point MyAIOne at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point MyAIOne at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to MyAIOne equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing MyAIOne config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in MyAIOne.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ MyAIOne's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("MyAIOne can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: myai claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: myai claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing MyAIOne configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in MyAIOne (use myai claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      myai setup           — full or quick (auto-detected)
      myai setup model     — just model/provider
      myai setup tts       — just text-to-speech
      myai setup terminal  — just terminal backend
      myai setup gateway   — just messaging platforms
      myai setup tools     — just tool configuration
      myai setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│  Let's configure your MyAIOne Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have MyAIOne configured.")
        print()
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'myai setup' again when ready.")
            return
        elif 2 <= choice <= 6:
        ...
        setup_mode = prompt_choice("How would you like to set up MyAIOne?", [
...
def _resolve_myai_chat_argv() -> Optional[list[str]]:
    ...
    try:
        if importlib.util.find_spec("myai_cli") is not None:
            return [sys.executable, "-m", "myai_cli.main", "chat"]
    except Exception:
        pass
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'myai setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    myai setup")
    if gateway_choice != 0:
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'myai setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect MyAIOne to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'myai setup gateway'.")
