...
def show_status(args):
    ...
    try:
        from myai_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: myai model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: myai model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: myai model)"
    ...
    if not terminal_env:
        ...
        # (myai status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        from myai_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    print(color("  Run 'myai doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'myai setup' to configure", Colors.DIM))
    print()
