...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or myai -c.",
    ...
    "Ctrl+Z suspends MyAIOne to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "myai -c resumes your most recent CLI session. myai -c \"project name\" resumes by title.",
    "myai -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "myai -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "myai chat -t web,terminal enables only specific toolsets for a focused session.",
    "myai chat -s github-pr-workflow preloads a skill at launch.",
    "myai chat -q \"query\" runs a single non-interactive query and exits.",
    "myai chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "myai chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "myai --yolo bypasses all dangerous command approval prompts for the entire session.",
    "myai chat --source telegram tags the session for filtering in myai sessions list.",
    "myai -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "myai doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "myai dump outputs a compact setup summary — great for bug reports.",
    "myai config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "myai config edit opens config.yaml in your default editor.",
    "myai config check scans for missing or stale configuration options.",
    "myai sessions browse opens an interactive session picker with search.",
    "myai sessions stats shows session counts by platform and database size.",
    "myai sessions prune --older-than 30 cleans up old sessions.",
    "myai skills search react --source skills-sh searches the skills.sh public directory.",
    "myai skills check scans installed hub skills for upstream updates.",
    "myai skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "myai skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "myai mcp add github --command npx adds MCP servers from the command line.",
    "myai mcp serve runs MyAIOne itself as an MCP server for other agents.",
    "myai auth add lets you add multiple API keys for credential pool rotation.",
    "myai completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "myai logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "myai backup creates a zip backup of your entire MyAIOne home directory.",
    "myai profile create coder creates an isolated profile that becomes its own command.",
    "myai profile create work --clone copies your current config and keys to a new profile.",
    "myai update syncs new bundled skills to ALL profiles automatically.",
    "myai gateway install sets up MyAIOne as a system service (systemd/launchd).",
    "myai memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as myai -w).",
    ...
    "MYAI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call MyAIOne tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'myai profile create coder' creates the 'coder' command.",
    "myai profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, MyAIOne prints a resume command with session ID and stats.",
    "myai sessions export backup.jsonl exports all sessions for backup or analysis.",
    "myai -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "myai skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: myai cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "MyAIOne runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "myai gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to MyAIOne via a local macOS server.",
    ...
    "MCP servers auto-generate toolsets at runtime — myai tools can toggle them per platform.",
    ...
    "myai chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "myai plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "myai acp runs MyAIOne as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MYAI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "myai login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'myai profile create ls' would be rejected.",
    "myai profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "myai claw migrate --dry-run previews OpenClaw migration without writing anything.",
