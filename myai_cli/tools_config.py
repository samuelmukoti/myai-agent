"""
Unified tool configuration for MyAIOne Agent.

`myai tools` and `myai setup tools` both enter this module.
...
from myai_cli.nous_subscription import (
...
from myai_cli.cli_output import (  # noqa: E402 — late import block
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from myai_cli.plugins import discover_plugins, get_plugin_toolsets
...
from myai_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _get_platform_tools(
    ...
    *,
    ...
    # A plugin toolset is "known" for a platform once `myai tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by myai tools — default enabled
                enabled_toolsets.add(pts)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `myai tools` and `myai setup tools`.
