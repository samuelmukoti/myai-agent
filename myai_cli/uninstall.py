...
def remove_path_from_shell_configs():
    """Remove MyAIOne PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# MyAIOne Agent" comment and following line
                if '# MyAIOne Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains myai_cli reference)
                content = wrapper.read_text()
                if 'myai_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def run_uninstall(args):
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL MyAIOne data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
    else:
        print("This will remove the MyAIOne code but keep your configuration and data.")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/samuelmukoti/myai-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
