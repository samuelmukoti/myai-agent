...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    MyAIOne supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``myai auth add <provider>`` command so the dashboard
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into MyAIOne even
    when they also have a separate MyAIOne-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "myai auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "myai auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "myai auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "myai auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from myai_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same MyAIOne-managed PKCE file
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from myai_cli.auth import clear_provider_auth
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both MyAIOne file AND credential pool.
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from myai_cli import auth as hauth
    if provider_id == "nous":
        from myai_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("MYAI_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from myai_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from myai_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from myai_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from myai_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from myai_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__MYAI_SESSION_TOKEN__="{_SESSION_TOKEN}";</script>'
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "MyAIOne Teal",  "description": "Classic dark teal — the canonical MyAIOne look"},
...
def _discover_dashboard_plugins() -> list:
    ...
    if os.environ.get("MYAI_ENABLE_PROJECT_PLUGINS"):
...
def start_server(
    ...
    print(f"  MyAIOne Web UI → http://{host}:{port}")
