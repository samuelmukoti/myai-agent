---
name: honcho
description: Configure and use Honcho memory with MyAIOne -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
...
# Honcho Memory for MyAIOne

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every MyAIOne profile its own peer identity while sharing a unified view of the user.
...
```bash
myai honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this MyAIOne profile's AI peer
...
Honcho models conversations as interactions between **peers**. MyAIOne creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this MyAIOne instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each MyAIOne run |
| `global` | Single session across all directories |

Manual override: `myai honcho map my-project-name`
...
Each MyAIOne profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
myai profile create coder --clone
# creates host block myai.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `myai.coder` host block in `honcho.json`
...
```bash
myai honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for MyAIOne when Honcho memory is active.
...
Run `myai honcho setup`. Ensure `memory.provider: honcho` is in `~/.hermes/config.yaml`.

### Memory not persisting across sessions
Check `myai honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `myai profile create <name> --clone`. For existing profiles: `myai honcho sync`.
...
| Command | Description |
|---------|-------------|
| `myai honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `myai honcho status` | Show resolved config, connection test, peer info for active profile |
| `myai honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `myai honcho disable` | Disable Honcho for the active profile |
| `myai honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `myai honcho peers` | Show peer identities across all profiles |
| `myai honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `myai honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `myai honcho sessions` | List known directory-to-session-name mappings |
| `myai honcho map <name>` | Map current working directory to a Honcho session name |
| `myai honcho identity` | Seed AI peer identity or show both peer representations |
| `myai honcho sync` | Create host blocks for all MyAIOne profiles that don't have one yet |
| `myai honcho migrate` | Step-by-step migration guide from OpenClaw native memory to MyAIOne + Honcho |
| `myai memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `myai memory status` | Show active memory provider and config |
| `myai memory off` | Disable external memory provider |
