...
description: Migrate a user's OpenClaw customization footprint into MyAIOne Agent. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: MyAIOne Agent (Nous Research)
license: MIT
metadata:
  hermes:
    tags: [Migration, OpenClaw, MyAIOne, Memory, Persona, Import]
    related_skills: [hermes-agent]
---

# OpenClaw -> MyAIOne Migration

Use this skill when a user wants to move their OpenClaw setup into MyAIOne Agent with minimal manual cleanup.
...
```bash
myai claw migrate              # Full interactive migration
...
myai claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `myai setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_myaione.py` to:

- import `SOUL.md` into the MyAIOne home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into MyAIOne memory entries
- merge OpenClaw command approval patterns into MyAIOne `command_allowlist`
...
- optionally copy the OpenClaw workspace instructions file into a chosen MyAIOne workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct MyAIOne destination
...
- `scripts/openclaw_to_myaione.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py`
...
MyAIOne CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in MyAIOne. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a MyAIOne workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing MyAIOne skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.hermes/skills/migration/openclaw-migration/scripts/openclaw_to_myaione.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of MyAIOne unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty MyAIOne targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean MyAIOne destination. Unsupported auth blobs must still be reported as skipped.
...
- MyAIOne persona state imported
- MyAIOne memory files populated with converted OpenClaw knowledge
