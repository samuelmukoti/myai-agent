---
name: telephony
description: Give MyAIOne phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives MyAIOne practical phone capabilities while keeping telephony out of the core tool list.
...
It does **not** turn MyAIOne into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to MyAIOne memory
...
### 1) "I want MyAIOne to own a real phone number"
Use **Twilio**.
...
- pairs well with MyAIOne `text_to_speech` plus a public file host or tunnel
...
```bash
myai skills search telephony
myai skills install official/productivity/telephony
```
...
Then save credentials into MyAIOne:
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is MyAIOne calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing MyAIOne's existing `text_to_speech` support.

Use this when:
- you want the call to use MyAIOne's configured TTS voice rather than Twilio `<Say>`
...
Recommended MyAIOne TTS -> Twilio Play workflow:

1. Generate the audio with MyAIOne `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask MyAIOne to create the message audio with `text_to_speech`
...
Important note:
- MyAIOne TTS is great for prerecorded outbound messages
...
- MyAIOne STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to MyAIOne memory.
...
- Do not store arbitrary third-party phone numbers in MyAIOne memory.
