...
```bash
myai honcho setup    # full interactive wizard (cloud or local)
myai memory setup    # generic picker, also works
```

Or manually:
```bash
myai config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated MyAIOne instances) |
| 2 | `~/.hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active MyAIOne profile: `hermes` (default) or `hermes.<profile>`.
...
| 4 | `per-session` strategy | MyAIOne session ID (`20260415_a3f2b1`) |
...
- **`per-directory`** — basename of `$PWD`. Opening myai in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — MyAIOne session ID (timestamp + hex). Every `myai` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple MyAIOne profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active MyAIOne profile: `hermes` (default) or `hermes.<profile>` (e.g. `myai -p coder` → host key `hermes.coder`).
...
| Command | Description |
|---------|-------------|
| `myai honcho setup` | Full interactive setup wizard |
| `myai honcho status` | Show resolved config for active profile |
| `myai honcho enable` / `disable` | Toggle Honcho for active profile |
| `myai honcho mode <mode>` | Change recall or observation mode |
| `myai honcho peer --user <name>` | Update user peer name |
| `myai honcho peer --ai <name>` | Update AI peer name |
| `myai honcho tokens --context <N>` | Set context token budget |
| `myai honcho tokens --dialectic <N>` | Set dialectic max chars |
| `myai honcho map <name>` | Map current directory to a session name |
| `myai honcho sync` | Create host blocks for all MyAIOne profiles |
