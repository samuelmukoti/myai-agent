...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into MyAIOne system prompt
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. MyAIOne session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — MyAIOne session_id ({timestamp}_{hex})
        ...
        # per-session: inherit MyAIOne session_id (new Honcho session each run)
