...
```bash
myai memory setup    # select "supermemory"
```

Or manually:

```bash
myai config set memory.provider supermemory
...
Use `{identity}` in the `container_tag` to scope memories per MyAIOne profile:
