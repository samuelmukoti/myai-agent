...
VERSION_FILE = REPO_ROOT / "myai_cli" / "__init__.py"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/samuelmukoti/myai-agent",
    ...
    lines.append(f"# MyAIOne Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for MyAIOne Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="MyAIOne Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  MyAIOne Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"MyAIOne Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"MyAIOne Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'MyAIOne Agent v{new_version} ({calver_date})' "
