...
# 5. Symlinks the 'myai' CLI command into a user-facing bin dir
...
# ============================================================================
# PATH setup — symlink myai into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up myai command..."

MYAI_BIN="$SCRIPT_DIR/venv/bin/myai"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MYAI_BIN" "$COMMAND_LINK_DIR/myai"
echo -e "${GREEN}✓${NC} Symlinked myai → $COMMAND_LINK_DISPLAY_DIR/myai"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# MyAIOne Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.myai/skills/
# ============================================================================
...
echo ""
echo "Syncing bundled skills to $MYAI_SKILLS_DIR ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MYAI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  myai status        # Check configuration"
if is_termux; then
    echo "  myai gateway       # Run gateway in foreground"
else
    echo "  myai gateway install # Install gateway service (messaging + cron)"
fi
echo "  myai cron list     # View scheduled jobs"
echo "  myai doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m myai_cli.main setup
fi
