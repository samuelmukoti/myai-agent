...
- Give it a name like "myaione-agent"
- Select scopes:
...
- Give it a title like "myaione-agent-<machine-name>"
...
elif [ -f ~/.myai/.env ] && grep -q "^GITHUB_TOKEN=" ~/.myai/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.myai/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
