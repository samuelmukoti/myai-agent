...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MYAI_HOME", os.path.expanduser("~/.myai")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MYAI_HOME", Path.home() / ".myai")) / "skills" / "red-teaming" / "godmode"
...
def _get_current_model() -> tuple:
    """Read current model and provider from MyAIOne config.yaml.
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart MyAIOne for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart MyAIOne for changes to take effect.")
