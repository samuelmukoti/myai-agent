...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "myai_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "myai_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from myai_cli.auth_commands import auth_reset_command
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from myai_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "myai_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("myai_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("myai_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("myai_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "myai_cli.auth_commands.time.time",
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from myai_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from myai_cli.auth import unsuppress_credential_source
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("myai_cli.auth._import_codex_cli_tokens", _fake_import)
