...
def _make_hermes_tree(root: Path) -> None:
    ...
    (root / "myai_state.db").write_bytes(b"fake-state")
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid MyAIOne backup."""
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only myai_state.db (old wrong name) is rejected."""
        from myai_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["myai_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MYAI_HOME", str(src_home))
        ...
        monkeypatch.setenv("MYAI_HOME", str(dst_home))
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MYAI_HOME", str(fake_home))
