...
- Unlimited timeout (MYAI_CRON_TIMEOUT=0)
- Backward compat: MYAI_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """MYAI_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """MYAI_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("MYAI_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """MYAI_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("MYAI_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("MYAI_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_hermes_time_importable(self):
        """myai_time should be importable when cron.scheduler loads."""
    ...
    def test_hermes_constants_importable(self):
        """myai_constants should be importable from cron context."""
        from myai_constants import get_myai_home
        assert callable(get_myai_home)
