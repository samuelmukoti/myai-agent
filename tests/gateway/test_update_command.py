...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _myai_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._myai_home", tmp_path), \
    ...
    async def test_no_myai_binary(self, tmp_path):
        ...
        assert "myai update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m myai_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "Starting MyAIOne update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m myai_cli.main
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        assert not (myai_home / ".update_exit_code").exists()
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (myai_home / ".update_pending.json").exists()
        assert not (myai_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (myai_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = myai_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (myai_home / ".update_pending.json").write_text(json.dumps(pending))
        (myai_home / ".update_output.txt").write_text("x" * 5000)
        (myai_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (myai_home / ".update_pending.json").write_text(json.dumps(pending))
        (myai_home / ".update_output.txt").write_text("Traceback: boom")
