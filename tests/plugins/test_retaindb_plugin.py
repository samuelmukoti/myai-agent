...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure MYAI_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", myai_home=str(tmp_path / ".hermes"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", myai_home=profile_home)
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir(exist_ok=True)
        monkeypatch.setenv("MYAI_HOME", str(myai_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", myai_home=str(myai_home))
        return p
