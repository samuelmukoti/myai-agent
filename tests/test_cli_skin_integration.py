...
from cli import MyAIOneCLI, _build_compact_banner, _rich_text_from_ansi
from myai_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("myai_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_uses_myaione_branding(self):
        set_active_skin("default")
        ...
        assert "MyAIOne" in banner
        assert "NOUS HERMES" not in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_myaione(self):
        set_active_skin("poseidon")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="v0.1.0 (test)"):
            banner = _build_compact_banner()

        assert "Poseidon Agent" in banner
        assert "MyAIOne" not in banner
        assert "NOUS HERMES" not in banner
