...
def myai_home(tmp_path, monkeypatch):
    """Provide an isolated MYAI_HOME for logging tests.
    ...
    """
    home = Path(os.environ["MYAI_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, myai_home):
        log_dir = myai_logging.setup_logging(myai_home=myai_home)
        assert log_dir == myai_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
        myai_logging.setup_logging(myai_home=myai_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        myai_logging.setup_logging(myai_home=myai_home, force=True)
    ...
    def test_custom_log_level(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, myai_home):
        myai_logging.setup_logging(
            myai_home=myai_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
    ...
    def test_writes_to_agent_log(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
        ...
        agent_log = myai_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
    ...
    def test_explicit_params_override_config(self, myai_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (myai_home / "config.yaml").write_text(yaml.dump(config))

        myai_logging.setup_logging(myai_home=myai_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, myai_home):
        """The custom record factory injects session_tag on all records."""
        myai_logging.setup_logging(myai_home=myai_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_myai_session_injector", False), (
            "Record factory should have _myai_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, myai_home):
        ...
        gw_log = myai_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, myai_home):
    ...
    def test_agent_log_still_receives_all(self, myai_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, myai_home):
    ...
    def test_no_session_tag_without_context(self, myai_home):
    ...
    def test_clear_session_context(self, myai_home):
    ...
    def test_session_context_thread_isolated(self, myai_home):
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, myai_home):
        myai_logging.setup_logging(myai_home=myai_home)
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, myai_home):
        level, max_size, backup = myai_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, myai_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, myai_home):
        import yaml
        config = {"model": "test"}
        (myai_home / "config.yaml").write_text(yaml.dump(config))
