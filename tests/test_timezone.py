"""
Tests for timezone support (myai_time module + integration points).
...
def _reset_hermes_time_cache():
    """Reset the myai_time module cache (replacement for removed reset_cache)."""
    myai_time._cached_tz = None
    myai_time._cached_tz_name = None
    myai_time._cache_resolved = False


# =========================================================================
# myai_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["MYAI_TIMEZONE"] = "UTC"
        result = myai_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["MYAI_TIMEZONE"] = "America/New_York"
        result = myai_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["MYAI_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="myai_time"):
            result = myai_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["MYAI_TIMEZONE"] = "Europe/London"
        tz = myai_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["MYAI_TIMEZONE"] = "Not/A/Timezone"
        tz = myai_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When MYAI_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) MYAI_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("MYAI_TIMEZONE=" + os.environ.get("MYAI_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "MYAI_TIMEZONE=NOT_SET" in result["output"], (
            "MYAI_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When MYAI_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("MYAI_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != MyAIOne tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to MyAIOne tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in MyAIOne tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of MyAIOne tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), MyAIOne is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when MyAIOne tz
        ...
        # Use a MyAIOne timezone far behind UTC so that the numeric wall time
        ...
        os.environ["MYAI_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hermes_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of MyAIOne timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["MYAI_TIMEZONE"] = "US/Eastern"
        _reset_hermes_time_cache()
