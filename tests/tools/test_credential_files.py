...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        myai_home.mkdir()
        (myai_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(myai_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        (myai_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        (myai_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        (myai_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        myai_home = tmp_path / ".hermes"
        (myai_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape MYAI_HOME."""
        monkeypatch.setenv("MYAI_HOME", str(tmp_path / ".hermes"))
        (tmp_path / ".hermes").mkdir()

        # Create a sensitive file one level above myai_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside myai_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside MYAI_HOME must still be registered."""
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        monkeypatch.setenv("MYAI_HOME", str(myai_home))
        (myai_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of MYAI_HOME must be allowed."""
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        subdir = myai_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside MYAI_HOME pointing outside must be rejected."""
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        monkeypatch.setenv("MYAI_HOME", str(myai_home))

        # Create a sensitive file outside myai_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside myai_home pointing outside
        symlink = myai_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes MYAI_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, myai_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape MYAI_HOME."""
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        monkeypatch.setenv("MYAI_HOME", str(myai_home))
        ...
        sensitive.write_text("{}")
        self._write_config(myai_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(myai_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside MYAI_HOME via config must still mount."""
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        monkeypatch.setenv("MYAI_HOME", str(myai_home))

        (myai_home / "oauth.json").write_text("{}")
        self._write_config(myai_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        (myai_home / "cache" / "documents").mkdir(parents=True)
        (myai_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("MYAI_HOME", str(myai_home))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        myai_home = tmp_path / ".hermes"
        myai_home.mkdir()
        # Use legacy dir name — get_myai_dir prefers old if it exists
        (myai_home / "document_cache").mkdir()
        (myai_home / "image_cache").mkdir()
        monkeypatch.setenv("MYAI_HOME", str(myai_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(myai_home / "document_cache") in host_paths
        assert str(myai_home / "image_cache") in host_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        myai_home = tmp_path / ".hermes"
        doc_dir = myai_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        myai_home = tmp_path / ".hermes"
        ss_dir = myai_home / "cache" / "screenshots"
