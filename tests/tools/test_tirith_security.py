...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in MYAI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_hermes_home(self):
        """_hermes_bin_dir must use MYAI_HOME, not hardcoded ~/.hermes."""
    ...
    def test_failure_marker_respects_hermes_home(self):
        """_failure_marker_path must use MYAI_HOME, not hardcoded ~/.hermes."""
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets MYAI_HOME; verify it's active."""
        myai_home = os.getenv("MYAI_HOME")
        assert myai_home is not None, "MYAI_HOME should be set by conftest"
        assert "hermes_test" in myai_home, "Should point to test temp dir"

    def test_get_myai_home_fallback(self):
        """Without MYAI_HOME set, falls back to ~/.hermes."""
        from tools.tirith_security import _get_myai_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove MYAI_HOME entirely
            os.environ.pop("MYAI_HOME", None)
            result = _get_myai_home()
