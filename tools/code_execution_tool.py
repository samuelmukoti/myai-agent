...
Lets the LLM write a Python script that calls MyAIOne tools via RPC,
...
"""Auto-generated MyAIOne tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["MYAI_RPC_SOCKET"])
...
"""Auto-generated MyAIOne tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("MYAI_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"MYAI_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("MYAI_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
    ...
    try:
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["MYAI_RPC_SOCKET"] = sock_path
        ...
        # MYAI_TIMEZONE is an internal MyAIOne setting and must not leak
        # into child processes.
        _tz_name = os.getenv("MYAI_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("MYAI_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {MYAI_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.myai/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.myai/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call MyAIOne tools programmatically. "
