...
def register_credential_file(
    ...
    *relative_path* is relative to ``MYAI_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside MYAI_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    myai_home = _resolve_hermes_home()

    # Reject absolute paths — they bypass the MYAI_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to MYAI_HOME)",
            relative_path,
        )
        return False

    host_path = myai_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape MYAI_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, myai_home)
...
# Each tuple is (new_subpath, old_name) matching myai_constants.get_myai_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_myai_dir()`` for backward compatibility with old directory layouts.
    """
    from myai_constants import get_myai_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_myai_dir(new_subpath, old_name)
        if host_dir.is_dir():
