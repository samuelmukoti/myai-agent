...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.myai/.env values without failing Docker command execution."""
    try:
        from myai_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``MYAI_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("MYAI_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using MYAI_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.myai/sandboxes/). Non-persistent
