...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or MyAIOne exit
...
# display_myai_home imported lazily at call site (stale-module safety during hermes update)
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("MYAI_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (MYAI_INTERACTIVE=1).
    ...
    try:
        os.environ["MYAI_SPINNER_PAUSE"] = "1"
        ...
        if "MYAI_SPINNER_PAUSE" in os.environ:
            del os.environ["MYAI_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (MYAI_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("MYAI_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.myai/.env or environment variables."
        )
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("MYAI_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("MYAI_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("MYAI_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("MYAI_SESSION_USER_ID", "")
                        _gw_user_name = _gse("MYAI_SESSION_USER_NAME", "")
