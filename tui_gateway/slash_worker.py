"""Persistent slash-command worker — one MyAIOneCLI per TUI session.
...
def _run(cli: MyAIOneCLI, command: str) -> str:
...
def main():
    ...
    os.environ["MYAI_SESSION_KEY"] = args.session_key
    os.environ["MYAI_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = MyAIOneCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
