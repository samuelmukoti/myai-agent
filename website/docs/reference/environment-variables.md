...
| `MYAI_COPILOT_ACP_COMMAND` | Override Copilot ACP CLI binary path (default: `copilot`) |
| `COPILOT_CLI_PATH` | Alias for `MYAI_COPILOT_ACP_COMMAND` |
| `MYAI_COPILOT_ACP_ARGS` | Override Copilot ACP arguments (default: `--acp --stdio`) |
...
| `MYAI_GEMINI_PROJECT_ID` | GCP project ID for paid Gemini tiers (free tier auto-provisions) |
...
| `MYAI_QWEN_BASE_URL` | Qwen Portal base URL override (default: `https://portal.qwen.ai/v1`) |
...
| `MYAI_MODEL` | Override model name at process level (used by cron scheduler; prefer `config.yaml` for normal use) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `MYAI_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `MYAI_LOCAL_STT_LANGUAGE` | Default language passed to `MYAI_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
...
| Variable | Description |
|----------|-------------|
| `MYAI_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `kilocode`, `xiaomi`, `arcee`, `alibaba`, `deepseek`, `nvidia`, `ollama-cloud`, `xai` (alias `grok`), `google-gemini-cli`, `qwen-oauth`, `bedrock`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
| `MYAI_PORTAL_BASE_URL` | Override Nous Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override Nous inference API URL |
| `MYAI_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `MYAI_NOUS_TIMEOUT_SECONDS` | HTTP timeout for Nous credential / token flows |
| `MYAI_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `MYAI_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `MYAI_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
| Variable | Description |
|----------|-------------|
| `MYAI_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
| `MYAI_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `MYAI_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `MYAI_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `MYAI_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `MYAI_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `MYAI_QUIET` | Suppress non-essential output (`true`/`false`) |
| `MYAI_API_TIMEOUT` | LLM API call timeout in seconds (default: `1800`) |
| `MYAI_STREAM_READ_TIMEOUT` | Streaming socket read timeout in seconds (default: `120`). Auto-increased to `MYAI_API_TIMEOUT` for local providers. Increase if local LLMs time out during long code generation. |
| `MYAI_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no chunks arrive within this window. |
| `MYAI_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `MYAI_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.hermes/plugins/` (`true`/`false`, default: `false`) |
| `MYAI_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
| `MYAI_EPHEMERAL_SYSTEM_PROMPT` | Ephemeral system prompt injected at API-call time (never persisted to sessions) |

## Interface

| Variable | Description |
|----------|-------------|
| `MYAI_TUI` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI when set to `1`. Equivalent to passing `--tui`. |
| `MYAI_AGENT_TUI_DIR` | Path to a prebuilt `ui-tui/` directory (must contain `dist/entry.js` and populated `node_modules`). Used by distros and Nix to skip the first-launch `npm install`. |

## Cron Scheduler

| Variable | Description |
|----------|-------------|
| `MYAI_CRON_TIMEOUT` | Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. |
| `MYAI_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
